...
### Nieuw
- `CryptoAnalysisBitvavo` (en `CryptoAnalysisBitvavoForDate`) lezen de te rapporteren indicatoren uit de nieuwe resource `src/main/resources/nl/debo/cryptodata/indicators`: één per regel, `#` voor commentaar, elke combinatie van `RSI`, `STOCH-RSI`, `K`, `D`, `MACD` en `MADR` (`MACD` staat voor MACD-lijn, signaal, histogram en de genormaliseerde MACD-stat). Een bestand met dezelfde naam naast de applicatie gaat vóór de resource, dus een run is om te bouwen zonder te hercompileren; een lege of verkeerd gespelde keuze stopt de run bij de start met een duidelijke melding. De keuze (nieuwe enum `Indicator`, doorgegeven aan `CryptoAnalysis` en `IndicatorAnalyzer`) werkt overal door: een weggelaten indicator hoeft niet compleet te zijn om een rij op te leveren (RSI alleen accepteert dus een jonge munt waar de MACD-warmup nog ontbreekt), staat als `NaN` in de `ResultRow` (`ResultRow.has(indicator)`), wordt niet op de console geprint, laat zijn CSV-cel leeg (de kolomindeling blijft gelijk; leeg leest terug als `NaN`, dus oude en nieuwe bestanden parsen hetzelfde), verdwijnt met zijn Range-kolom uit het XLSX (de rood/groen-opmaak schuift mee) en telt niet mee in de Score, die nu het gemiddelde is van alleen de gekozen 0..1-statistieken. Met alle zes aan zijn console, CSV en XLSX exact als voorheen.

### Veranderd
- `XslxPrinter` beschrijft zijn kolommen nu in één tabel (kop, breedte, opmaak, bijbehorende indicator, waarde, rood/groen-regel) in plaats van losse header-, breedte- en celindex-lijsten met hardgecodeerde kolomletters voor de voorwaardelijke opmaak; `XslxPrinter.write` krijgt de indicatorkeuze mee. `TestXslxPrinter` leidt de keuze af uit de CSV (wat ergens een waarde heeft, was gekozen).
- `analysis-pipeline.md` beschrijft de indicatorkeuze en wat elke stap met een weggelaten indicator doet.

## [14] - 2026-08-15

### Nieuw
- React-webapp in `webapp/` (Vite + lightweight-charts) die de lokale candle-opslag `output/klines-bitvavo/` als candlestick-grafiek toont: kies een munt en tijdvenster (1h t/m 1M); onder de grafiek staan de knoppen RSI, S-RSI en MACD die los van elkaar aan/uit kunnen. Elke knop toont het indicatorpaneel onder de candles en kleurt candles op een koopsignaal **goud** (RSI ≤ 30; StochRSI %K kruist %D omhoog met K ≤ 0,2; MACD-lijn kruist signaal omhoog). Met "all fire"/"any fires" bepaal je of alle ingeschakelde indicatoren tegelijk moeten vuren of één volstaat. Elke koopsignaal-candle opent een gesimuleerde trade van €1000 (instelbaar) op de slotkoers, mits er geen trade openstaat (vinkje "skip signal if trade is active", standaard aan; overgeslagen signalen krijgen een gouden rand — zonder vinkje opent elk signaal een eigen trade met eigen stop en doel); elke volgende candle wordt op zijn slotkoers beoordeeld: sluit hij 8% of meer onder de instap (stop) of 25% of meer erboven (doel), dan wordt op díe slotkoers verkocht — een candle die op −10% sluit boekt dus −10%. Bij kopen én verkopen gaat 0,25% kosten af. Drempels, bedrag per trade en kosten zijn instelbaar. Koop/verkoop-pijlen tonen het resultaat in euro's per trade, entry/stop/doel-lijnen markeren de open trade, de voettekst telt de gerealiseerde winst/verlies op (met het samengestelde percentage ter referentie) en "show log" toont alle trades. De keuze staat in de URL zodat een weergave herladen kan worden. De indicatorberekening (`webapp/src/indicators.js`) is een 1-op-1 port van de Java `Indicators` en geeft op hetzelfde candlevenster exact de rapportwaardes. Starten: `cd webapp && npm install && npm run dev` (zie `webapp/README.md`).

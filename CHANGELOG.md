...
### Nieuw
- `CryptoAnalysisBitvavo` (en `CryptoAnalysisBitvavoForDate`) lezen de te rapporteren indicatoren uit de nieuwe resource `src/main/resources/nl/debo/cryptodata/indicators`: één per regel, `#` voor commentaar, elke combinatie van `RSI`, `STOCH-RSI`, `K`, `D` en `MACD` (`MACD` staat voor MACD-lijn, signaal, histogram en de genormaliseerde MACD-stat; MADR staat altijd aan). Een bestand met dezelfde naam naast de applicatie gaat vóór de resource, dus een run is om te bouwen zonder te hercompileren; een lege of verkeerd gespelde keuze stopt de run bij de start met een duidelijke melding. De keuze (nieuwe enum `Indicator`, doorgegeven aan `CryptoAnalysis` en `IndicatorAnalyzer`) werkt overal door: een weggelaten indicator hoeft niet compleet te zijn om een rij op te leveren (RSI alleen accepteert dus een jonge munt waar de MACD-warmup nog ontbreekt), staat als `NaN` in de `ResultRow` (`ResultRow.has(indicator)`), wordt niet op de console geprint, laat zijn CSV-cel leeg (de kolomindeling blijft gelijk; leeg leest terug als `NaN`, dus oude en nieuwe bestanden parsen hetzelfde), verdwijnt met zijn Range-kolom uit het XLSX (de rood/groen-opmaak schuift mee) en telt niet mee in de Score, die nu het gemiddelde is van alleen de aanwezige 0..1-statistieken. Standaard staan alle vijf aan, en dan zijn console, CSV en XLSX exact als voorheen.
